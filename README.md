...
Empfehlung:
- Der Bot startet jetzt immer einen Health-Port (`PORT` oder Fallback `10000`), damit Render-Portscan nicht mehr hängt.
...
### Website in Google sichtbar machen
Wenn die Seite bei Google noch nicht erscheint, ist das normal kurz nach Deployment.

Wir haben jetzt SEO-Basis eingebaut:
- indexierbare Startseite `/`
- `robots.txt`
- `sitemap.xml`
- Meta Description + OpenGraph

Tipp:
1. URL in Google Search Console eintragen
2. Sitemap hinterlegen: `/sitemap.xml`
3. Auf Indexierung warten (kann Stunden bis Tage dauern)
